---
type: index
project: Prospector CRM
status: active
updated: 2026-09-21
tags:
  - prospector
  - crm
  - index
---

# Prospector CRM

> Sistema pessoal de prospecção de Henrique Bezerra. A inteligência prepara; Henrique verifica, decide e executa qualquer contato externo.

## Começar por aqui

- [[Product Vision]] — para quem o produto existe e qual problema resolve
- [[Operating System]] — rotina diária e semanal de uso
- [[Roadmap]] — agora, próximo e depois
- [[CRM Snapshot]] — retrato gerado a partir dos dados reais do sistema

## Produto e engenharia

- [[Architecture Map]]
- [[Data Model]]
- [[Agent And AI Guardrails]]
- [[Agent Skills and Capabilities]]
- [[Decision Log]]
- [[Research Inbox]]

## Comercial e Playbooks

- [[Sales Playbook]] — scripts oficiais do Henrique e 3 variações de abordagem
- [[Playbook - Central de Abordagens e Modelos de Conversao]] — as 3 perguntas de impacto, canais de contato e webhooks
- [[Playbook - Demonstracoes e Mockups com Lovable]] — criação de protótipos de sites ao vivo
- [[Playbook - Auditoria de Sites e Quebra de Objeções]] — auditorias técnicas de velocidade e SEO local
- [[Playbook - Propostas Comerciais e Fechamento]] — propostas em PDF e contratos
- [[Playbook - Rotina Diaria e Find Skills]] — rotina matinal e busca de novas skills
- [[Playbook - Biblioteca de Prompts das Skills]] — comandos e prompts prontos para copiar e colar
- [[Customer Discovery]]
- [[Weekly Review]]

## Integrações

- [[Notion Integration Plan]]

## Templates

- [[Template - Experiment]]
- [[Template - Interview]]
- [[Template - Weekly Review]]

## Convenções do vault

- Uma nota deve representar uma decisão, aprendizado, rotina ou pergunta útil.
- Use links internos entre notas em vez de duplicar contexto.
- Registre hipóteses como hipóteses; não transforme pesquisa incompleta em fato.
- Informações operacionais de leads vivem no CRM. O vault guarda raciocínio e aprendizado.
